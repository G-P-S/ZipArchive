...
{
	NSString	*	zipFilePath=NULL;
	
	if (![zipFileURL isFileURL]) {
		return NO;
	}
	zipFilePath = [zipFileURL path];
	if (NULL==zipFilePath) {
		return NO;
	}
	  ...
	  statusDisplay:(BOOL (^)(float percentDone))statusDisplay
{
	...
	if (ret != UNZ_END_OF_LIST_OF_FILE) return success;
	
	ret = unzGoToFirstFile( _unzFile );
	...
	do{
		...
		{// contains a DOS-style path
			strPath = [strPath stringByReplacingOccurrencesOfString:@"\\" withString:@"/"];
		}
		
		NSURL * fullPath = [destinationURL URLByAppendingPathComponent:strPath];
		
		if( isDirectory )
			[fman createDirectoryAtPath:[fullPath path] withIntermediateDirectories:YES attributes:nil error:nil];
		else
			[fman createDirectoryAtPath:[[fullPath URLByDeletingLastPathComponent]path] withIntermediateDirectories:YES attributes:nil error:nil];
		if( [fman fileExistsAtPath:[fullPath path]] && !isDirectory && !overwrite )
		{
			...
			[fman createFileAtPath:[fullPath path] contents:NULL attributes:NULL];
		}

		
		NSError		 *	fhError=NULL;
		NSFileHandle * fileHandle = NULL;
		
		if (!isDirectory) {
			fileHandle = [NSFileHandle fileHandleForWritingToURL:fullPath error:&fhError];
			
			if (fhError) {
				NSLog(@"Error fullpath %@ %@",[fullPath path],fhError);
			}
			while( continueUnzip && fileHandle )
			{
				read=unzReadCurrentFile(_unzFile, buffer, 4096);
				if( read > 0 )
				{
					float percent;
					[fileHandle writeData:[NSData dataWithBytesNoCopy:buffer length:read freeWhenDone:NO]];
					totalBytesProcessed += read;
					percent = (float)totalBytesProcessed/(float)totalPackageSize;
					
					continueUnzip = statusDisplay( percent );
				}
				else if( read<0 )
				{
		...
		if( fileHandle )
		{
			...
			if( attr )
			{
				...
				if( ![[NSFileManager defaultManager] setAttributes:attr ofItemAtPath:[fullPath path] error:nil] )
				{
		...
		ret = unzGoToNextFile( _unzFile );
		[pool drain];
	}while( continueUnzip && ret==UNZ_OK && ret!=UNZ_END_OF_LIST_OF_FILE );
	return success;
}
